...
local function isWhitespace(chr)
    return chr==" " or chr=="\u{2800}"
end

local function getBg(x,y)
    return ({gpu.get(x,y)})[3]
end

local function lerp(a,b,c) return a*(1-c)+b*c end

local function blend(n1,n2,c)
    local c1 = {n1>>16,(n1>>8)&255,n1&255}
    local c2 = {n2>>16,(n2>>8)&255,n2&255}
    local c3 = {lerp(c1[1],c2[1],c)//1,lerp(c1[2],c2[2],c)//1,lerp(c1[3],c2[3],c)//1}
    return (c3[1]<<16)|(c3[2]<<8)|c3[3]
end
        ...
        ["show"]=function(self,x,y)
            ...
            for j=1,self.height do
                for i=1,self.width do
                    ...
                    local alpha = self.data[4][idx]
                    if alpha==1 and isWhitespace(self.data[1][idx]) then goto continue end
                    gpu.setForeground(self.data[2][idx])
                    if alpha==1 then
                        gpu.setBackground(getBg(i+x-1,j+y-1))
                    elseif alpha==0 then
                        gpu.setBackground(self.data[3][idx])
                    else
                        gpu.setBackground(blend(self.data[3][idx],getBg(i+x-1,j+y-1),alpha))
                    end
...
local function readComplex(file,width,height,lengthOffset,positionOffset)
    ...
    for alpha = 1, file:readBytes(1) + lengthOffset do
        ...
        for symbol = 1, file:readBytes(2) + lengthOffset do
            local currentSymbol = file:readUnicodeChar()

            for background = 1, file:readBytes(1) + lengthOffset do
                local currentBackground = to24Bit(file:readBytes(1))

                for foreground = 1, file:readBytes(1) + lengthOffset do
                    local currentForeground = to24Bit(file:readBytes(1))

                    for y = 1, file:readBytes(1) + lengthOffset do
                        local currentY = file:readBytes(1)

                        for x = 1, file:readBytes(1) + lengthOffset do
                            img:set(
                                file:readBytes(1) + positionOffset,
                                currentY + positionOffset,
                                currentSymbol,
                                currentForeground,
                                currentBackground,
                                currentAlpha
